...
import axios, { CancelTokenSource } from 'axios';
import apiResponse from '@/app/utils/addNinjaOtpJobs';
...
export default class NinjaOtpQueue {
    ...
    private async addJobsIfNotExists(jobsArray: NinjaOtpJob[], newJobsData: Array<{ id: string, operator: string }>, apiKey: string) {
    ...
    private async fetchNumber(job: NinjaOtpJob, cancelToken: any) {
        const res = await axios.get('/api/proxy/ninjaotp', {
            cancelToken,
            params: {
                apiKey: job.apiKey,
    ...
    private async cancelNumber(job: NinjaOtpJob, accessId: string) {
        const res = await axios.get('/api/proxy/ninjaotp', {
            params: {
                apiKey: job.apiKey,
                action: 'setStatus',
        ...
        if (res.data.data !== "ACCESS_CANCEL" && res.data.data !== "ACCESS_CANCEL_ALREADY") {
            this.addLog(`Server ${job.serverId}: Number wasn't cancelled properly. 
                Please cancel the number and restart the fetching`, 'error');
            // this.stop(); // disabled only for NinjaOtp service
            throw new Error(`Cancellation failed: ${res.data.data}`);
        }
    }
}